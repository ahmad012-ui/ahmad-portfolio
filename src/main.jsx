...
function SocialIcons(){return SOCIAL_LINKS.filter(s=>MAIN_SOCIAL.includes(s.key)).sort((a,b)=>MAIN_SOCIAL.indexOf(a.key)-MAIN_SOCIAL.indexOf(b.key)).map(s=>{const Icon=s.Icon;return <a key={s.key} className="social-icon" href={s.href} aria-label={s.label} title={s.label} target={s.key==='email'?undefined:'_blank'} rel={s.key==='email'?undefined:'noreferrer'}><Icon/></a>})}
const projects=[
...
function App(){
 const [menu,setMenu]=useState(false),[active,setActive]=useState('home'),[selectedProject,setSelectedProject]=useState(null);
 useEffect(()=>{
 ...
 useEffect(()=>{
  ...
  const closeOnEscape=e=>e.key==='Escape'&&setSelectedProject(null);
 ...
 return <div className="site">
  ...
  {selectedProject&&<div className="project-modal-backdrop" role="presentation" onMouseDown={e=>e.target===e.currentTarget&&setSelectedProject(null)}><div className="project-modal" role="dialog" aria-modal="true" aria-labelledby="project-modal-title"><button className="project-modal-close" onClick={()=>setSelectedProject(null)} aria-label="Close project details"><X/></button><div className="project-modal-image"><img src={selectedProject.image} alt=""/></div><div className="project-modal-content"><p className="eyebrow">{selectedProject.n} — {selectedProject.type}</p><h2 id="project-modal-title">{selectedProject.title}</h2><p>{selectedProject.desc}</p><div className="tags">{selectedProject.stack.map(s=><span key={s}>{s}</span>)}</div><a className="btn solid" href={selectedProject.repo} target="_blank" rel="noreferrer">VIEW ON GITHUB <ExternalLink/></a></div></div></div>}
